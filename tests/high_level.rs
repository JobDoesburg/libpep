...
fn test_encode() {
    ...
    let pseudo = Pseudonym::random(&pseudo_context1, rng);
    let enc_pseudo = encrypt_pseudonym(&pseudo, &session1_public, rng);

    println!("{}", pseudo.encode());
    println!("{}", enc_pseudo.encode());
...
fn test_high_level_flow() {
    ...
    assert_eq!(pseudonymized_dec.context, pseudo_context2);
    ...
    assert_eq!(rev_pseudonymized_dec.context, pseudo_context1);
...
//         make_session_keys(&global_secret, &enc_context2, &enc_secret);
//
//     let pseudo_1 = Pseudonym::random(&pseudo_context1, rng);
